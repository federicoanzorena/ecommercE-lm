...
from .modelos import Rol, TokenRefresco, TokenVerificacion, Usuario, UsuarioRol
...
class ServicioAutenticacion:
    ...
    def registrar_usuario(
        ...
        if len(password) > configuracion.longitud_maxima_password:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"La contraseña no puede superar los {configuracion.longitud_maxima_password} caracteres",
            )
    ...
    def restablecer_password(db: Session, token: str, nueva_password: str) -> dict:
        ...
        if len(nueva_password) > configuracion.longitud_maxima_password:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"La contraseña no puede superar los {configuracion.longitud_maxima_password} caracteres",
            )
