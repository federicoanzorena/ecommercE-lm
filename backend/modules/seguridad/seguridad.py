...
def _claims_base(usuario_id: UUID) -> dict:
    return {
        "sub": str(usuario_id),
        "iss": configuracion.jwt_issuer,
        "aud": configuracion.jwt_audience,
    }
...
def crear_access_token(usuario_id: UUID) -> str:
    ...
    payload = {**_claims_base(usuario_id), "exp": expiracion, "tipo": "access", "jti": str(uuid4())}
...
def crear_refresh_token(usuario_id: UUID) -> str:
    ...
    payload = {**_claims_base(usuario_id), "exp": expiracion, "tipo": "refresh", "jti": str(uuid4())}
    return jwt.encode(payload, configuracion.secret_key, algorithm=configuracion.algoritmo_jwt)
...
def crear_token_verificacion(usuario_id: UUID, tipo: str) -> str:
    ...
    payload = {**_claims_base(usuario_id), "exp": expiracion, "tipo": tipo, "jti": str(uuid4())}
    return jwt.encode(payload, configuracion.secret_key, algorithm=configuracion.algoritmo_jwt)
...
def decodificar_token(token: str) -> dict:
    ...
    if payload.get("iss") != configuracion.jwt_issuer:
        raise JWTError("Emisor inválido")
    return payload
